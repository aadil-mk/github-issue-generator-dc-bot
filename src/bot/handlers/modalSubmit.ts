...
export const handleModalSubmit = async (interaction: ModalSubmitInteraction) => {
    ...
    const requestType = fields.getTextInputValue("requestType");
    ...
    try {
        ...
        const devIds = String(ENV.DEVELOPER_IDS).split(",")
            .map((id) => id.trim())
            .filter(Boolean);

        const embed = new EmbedBuilder()
            .setTitle("✅ Issue Created Successfully")
            .setDescription(`Your issue **${issueTitle}** has been forwarded to our developers.\nThank you for your feedback!`)
                ...
                try {
                    ...
                    if (devUser) {
                        const dmEmbed = new EmbedBuilder()
                            ...
                            .setDescription(`A new issue has just been opened by <@${interaction.user.id}>!\n\n**${issueTitle}**\n\n[Click securely to view this issue on GitHub](${res.data.html_url})`)
                            .setColor(COLOR.BLUE);

                        await devUser.send({ embeds: [dmEmbed] });
                    }
                } catch (dmError) {
                    logger.error(`Failed to dispatch DM to Developer ID: ${devId}`, dmError);
                }
            }
        }
    } catch (error) {
